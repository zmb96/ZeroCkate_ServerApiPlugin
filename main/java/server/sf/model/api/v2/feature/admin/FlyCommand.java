...
public final class FlyCommand implements CommandExecutor, TabCompleter {
    ...
    public FlyCommand(AdminStateManager st) {
        this.st = st;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("sf.admin.fly")) {
        ...
        if (now) st.setGod(target, false);
        SF.sf().msg(sender, "§a" + target.getName() + " 飞行 §e" + (now ? "已开启" : "已关闭"));
        if (explicit) SF.sf().msg(target, "§a你的飞行模式已 §e" + (now ? "开启" : "关闭"));
        return true;
    }
