...
public class PerformanceManager {
    ...
    public void start() {
        SF.sf().info("[Perf] Performance system starting...");

        monitorTaskId = SF.sf().tick().runTimer(this::runMonitor, monitorIntervalTicks);
        cleanupTaskId = SF.sf().tick().runTimer(this::runCleanupSync, cleanupIntervalTicks);
        throttleTaskId = SF.sf().tick().runTimer(this::runAutoThrottle, monitorIntervalTicks);
        ...
        SF.sf().info("[Perf] Memory monitor: " + memoryMonitor);
        SF.sf().info("[Perf] Chunk manager: " + chunkManager);
        SF.sf().info("[Perf] Entity cleaner: " + entityCleaner);
        SF.sf().info("[Perf] Auto throttle: " + autoThrottle);
        SF.sf().info("[Perf] Performance system initialized");
    }

    public void shutdown() {
        if (monitorTaskId != -1) SF.sf().tick().cancel(monitorTaskId);
        if (cleanupTaskId != -1) SF.sf().tick().cancel(cleanupTaskId);
        if (throttleTaskId != -1) SF.sf().tick().cancel(throttleTaskId);
        SF.sf().info("[Perf] Performance system stopped");
    }
    ...
    private void runMonitor(long sfTick) {
        try {
            ...
            SF.sf().error("[Perf] Monitor error", t);
        }
    }
    ...
    private void checkMemory() {
        ...
        if (ratio >= memoryGcThreshold) {
            SF.sf().warn("[Perf] Memory critical: %.1f%% (%d/%d MB), triggering GC",
                    ratio * 100, lastUsedMemory / 1048576, lastMaxMemory / 1048576);
            System.gc();
            gcTriggeredTotal++;
        } else if (ratio >= memoryWarnThreshold) {
            SF.sf().warn("[Perf] Memory high: %.1f%% (%d/%d MB)",
                    ratio * 100, lastUsedMemory / 1048576, lastMaxMemory / 1048576);
        }
    }
    ...
    private void runCleanupSync(long sfTick) {
        SF.sf().tick().runSync(() -> {
            try {
                ...
                SF.sf().error("[Perf] Cleanup error", t);
            }
        });
    }
    ...
    private void cleanIdleChunks() {
        ...
        if (unloaded > 0) {
            ...
            SF.sf().info("[Perf] Unloaded %d idle chunks (total: %d)", unloaded, chunksUnloadedTotal);
        }
    }
    ...
    private void cleanEntities() {
        ...
        if (removed > 0) {
            ...
            SF.sf().info("[Perf] Cleaned %d entities (total: %d)", removed, entitiesRemovedTotal);
        }
    }
    ...
    private void runAutoThrottle(long sfTick) {
        ...
        if (tps < tpsCriticalThreshold) {
            ...
            SF.sf().warn("[Perf] TPS critical (%.1f), throttling view/sim distance", tps);
            ...
            SF.sf().tick().runSync(() -> {
                for (World w : Bukkit.getWorlds()) {
                    try {
                        ...
                        SF.sf().error("[Perf] Failed to adjust world distance", t);
                    }
                }
            });
            currentViewDistance = targetViewDist;
            currentSimulationDistance = targetSimDist;
            SF.sf().info("[Perf] View distance -> %d, Simulation distance -> %d", targetViewDist, targetSimDist);
        }
    }
    ...
    public void manualGc() {
        SF.sf().info("[Perf] Manual GC triggered");
        long before = Runtime.getRuntime().freeMemory();
        System.gc();
        long after = Runtime.getRuntime().freeMemory();
        SF.sf().info("[Perf] GC freed %d MB", (after - before) / 1048576);
        gcTriggeredTotal++;
    }
    ...
    public void manualChunkCleanup() {
        SF.sf().tick().runSync(() -> {
            ...
            cleanIdleChunks();
            collectStats();
            SF.sf().info("[Perf] Manual chunk cleanup done: %d -> %d", before, lastChunkCount);
        });
    }
    ...
    public void manualEntityCleanup() {
        SF.sf().tick().runSync(() -> {
            ...
            cleanEntities();
            collectStats();
            SF.sf().info("[Perf] Manual entity cleanup done: %d -> %d", before, lastEntityCount);
        });
    }
